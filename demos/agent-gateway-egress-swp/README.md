...
| **Agent Gateway Authz Policy** | Overrides the gateway's internal default-deny behavior and allows all traffic to flow to the customer VPC | `google_network_security_authz_policy.allow_all` in `modules/agent-gateway/main.tf` |
...
1. **Agent Gateway `AuthzPolicy` (Allow-All)** — The Google-managed Agent Gateway operates an internal Envoy proxy with a default-deny policy. Because CUJ2 focuses on **egress network governance via the customer SWP** and does not deploy IAP/Model Armor authorization extensions, we attach `google_network_security_authz_policy.allow_all` (`action = "ALLOW"`, `when = "true"`) directly to the Agent Gateway. This overrides the internal `default_denied` drop and allows all traffic to pass freely through the PSC-Interface into the customer VPC.
